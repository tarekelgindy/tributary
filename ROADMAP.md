...
- [x] Per-outlet **attention distribution** vs. the event universe → outlet *agenda fingerprints*. *(2026-06-11 — share of outlet's in-window items per story + best feed position, both raw counts (P5).)*
...
- [ ] Calibrate the clustering thresholds Gate-1-style: ~40 labeled headline pairs (same-story / related / different) sampled from real captures → calibrate story-join 0.62 and adjacency 0.42; log the result here. *(Instrument ready 2026-06-11: `gen_agenda_pairs.py` wrote 40 similarity-stratified pairs to `agenda/calibration_pairs_blind.json` — sims withheld, key separate. Waiting on Tarek's labels, then `--score`.)*
