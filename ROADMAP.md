...
- [x] **Ask-your-own-question**: a question box on the homepage matches pasted claims/events against the published corpus (lexical scoring over `gallery/search_index.json`, rebuilt by `publish.py`); hits serve instantly (match-and-serve, finally visible); misses fall through to **request-a-trace** with two explicit paths (origin trace / event framing map). Fulfillment is **automatic on request** (Tarek's call, 2026-07-05): the workflow runs the moment a template issue opens — origin traces AND lean event framing maps — publishes, posts the permalink (~5–10 min), and closes the request. Spend control moved from human approval to a **daily cap** (10 auto-runs/UTC-day, counted from fulfillment commits; overflow queues politely; the `approved-trace` label force-runs past the cap). Every request is Gate-3 signal and grows the corpus where demand actually is. *(2026-07-05 — secret + labels configured by Tarek; dry run still pending. Scale note: the GitHub-Actions executor is deliberately disposable scaffolding behind a stable interface (request in → status/permalink out); the B-phase Cloudflare front removes GitHub from the user's view, and a container service replaces Actions when volume justifies it.)*
...
- [x] The turnaround promise proven once end-to-end: in-window request → Worker (rate-limited) → dispatch → generation → permalink in ~9 minutes, status callback confirmed, trace auto-entered the search index (19a388269b88, microwaving/nutrients — honest earliest-found status, 6 critics, 6 quote-verified). The Cloudflare front door is DEPLOYED and live. *(2026-07-05)*
