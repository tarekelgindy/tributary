        ...
        if verbose and views and len(views) >= 2:
            ...
            if primary_url:
                try:
                    ...
                    if str(primary_content).startswith("[Error"):
                        primary_content = ""
                except Exception:
                    primary_content = ""

            shared_data = await tracer.analyze_shared_foundation(
                event_text=fact_text,
                views=views,
                primary_source_url=primary_url,
                primary_source_content=str(primary_content) if primary_content else "",
            )
            result["shared_foundation"] = shared_data

            print_separator("SHARED FOUNDATION")

            verified = shared_data.get("verified_facts", [])
            unverified = shared_data.get("unverified_facts", [])
            disagreements = shared_data.get("points_of_disagreement", [])
            ...
            if verified:
                print(f"  {green}✓ VERIFIED FACTS{reset_c} (supported by primary source)")
                for f in verified:
                    print(f"    {green}•{reset_c} {f.get('fact', '')}")
                print()

            if unverified:
                print(f"  {yellow}? UNVERIFIED CLAIMS{reset_c} (agreed across views but not "
                      f"directly in primary source)")
                for f in unverified[:5]:
                    print(f"    {yellow}•{reset_c} {f.get('fact', '')}")
                if len(unverified) > 5:
                    print(f"    ... and {len(unverified) - 5} more")
                print()

            if disagreements:
                print(f"  {cyan}↯ WHERE VIEWS DIVERGE{reset_c}")
                for d in disagreements[:5]:
                    topic = d.get("topic", "")
                    positions = d.get("view_positions", [])
                    print(f"    {cyan}•{reset_c} {topic}")
                    for p in positions[:3]:
                        print(f"        {p.get('view', '')}: {p.get('position', '')[:120]}")
                if len(disagreements) > 5:
                    print(f"    ... and {len(disagreements) - 5} more points of disagreement")
...
async def run_trace(args):
    """Build and display a provenance tree."""
    from agent import SourceTracer, ContentLabel, Claim
    from search_anthropic import AnthropicSearchAdapter
    ...
    try:
        # Determine the claim text to trace
        claim_text = args.claim
        if args.url and not args.claim:
            from ingestors import ContentExtractor
            print(f"\nIngesting: {args.url}")
            extractor = ContentExtractor()
            content = await extractor.extract(args.url)
            await extractor.close()
            if not content or content.startswith("[Error"):
                print(f"Failed to extract content from URL: {content}")
                return

            print("Extracting most significant claim from content...")
            claims = await tracer.extract_claims(content)
            # Pick the highest-priority traceable claim
            traceable = [c for c in claims if c.label in (
                ContentLabel.FACT, ContentLabel.NARRATIVE, ContentLabel.STUDY
            )]
            if not traceable:
                print("No traceable claims found in the content.")
                return
            chosen = traceable[0]
            claim_text = chosen.text
            print(f"\nTracing: {claim_text}")

        # Build the trace tree
        days_back = args.days_back if args.days_back > 0 else None
        tree = await tracer.trace_tree(
            claim_text=claim_text,
            depth=args.depth,
            days_back=days_back,
            include_social=args.social,
            multi_frame=args.multi_frame,
            discover_framings=args.discover_framings,
            custom_framings=args.also_trace,
        )

        # Print summary
        print_separator("TRACE COMPLETE")
        print(f"  Total sources: {len(tree.nodes)}")
        print(f"  Origins identified: {len(tree.origins)}")
        print(f"  Main thread length: {len(tree.main_thread)}")
        print(f"  Cost: ${tree.cost_usd:.3f}")

        if tree.origins:
            print(f"\n  ORIGINS (no parents):")
            for oid in tree.origins[:5]:
                node = next((n for n in tree.nodes if n.node_id == oid), None)
                if node:
                    date = node.published_date or "?"
                    print(f"    [{date}] {node.outlet}: {node.title[:60]}")

        if tree.main_thread:
            print(f"\n  MAIN THREAD (path to most-amplified version):")
            for i, nid in enumerate(tree.main_thread):
                node = next((n for n in tree.nodes if n.node_id == nid), None)
                if node:
                    arrow = "  → " if i > 0 else "    "
                    date = node.published_date or "?"
                    print(f"  {arrow}[{date}] {node.outlet}: {node.title[:50]}")

        if tree.framings:
            print(f"\n  FRAMINGS ({tree.framing_mode}):")
            for f in tree.framings:
                count = sum(
                    1 for n in tree.nodes
                    if n.framings.get(f.framing_id, 0) >= 0.5
                )
                marker = " (neutral)" if f.is_neutral else (
                    " (your input)" if f.is_original_input else ""
                )
                print(f"    [{f.label}{marker}] {count}/{len(tree.nodes)} sources carry this")
                if f.framing_text:
                    print(f"      \"{f.framing_text[:90]}\"")

        # Save the tree
        import os
        from datetime import datetime
        date_str = datetime.now().strftime("%Y-%m-%d")
        traces_dir = os.path.join("traces", date_str)
        os.makedirs(traces_dir, exist_ok=True)
        claim_hash = hashlib.sha256(claim_text.encode()).hexdigest()[:10]
        default_path = os.path.join(traces_dir, f"trace_{claim_hash}.json")

        with open(default_path, "w") as f:
            json.dump(tree.to_dict(), f, indent=2)
        print(f"\n  Tree saved to: {default_path}")

        # Optional explicit --json export
        if args.json:
            with open(args.json, "w") as f:
                json.dump(tree.to_dict(), f, indent=2)
            print(f"  Also saved to: {args.json}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Build a full provenance tree for a claim (use with --claim or --url)"
    )
    parser.add_argument(
        "--depth", type=str, default="standard",
        choices=["quick", "standard", "deep"],
        help="Trace depth for --trace mode (default: standard)"
    )
    parser.add_argument(
        "--multi-frame", action="store_true",
        help="(With --trace) Generate ideologically-distinct framings of the claim and "
             "tag each source with which framings it carries. Adds ~$1-2 to trace cost."
    )
    parser.add_argument(
        "--discover-framings", action="store_true",
        help="(With --trace) Discover framings empirically from the corpus rather than "
             "generating them. More accurate, slower, more expensive."
    )
    parser.add_argument(
        "--also-trace", type=str, nargs="+",
        help="(With --trace) Additional custom framings to trace alongside the main claim. "
             "E.g., --also-trace 'the system is broken' 'regulatory capture'"
    )
    parser.add_argument(
        "--days-back", type=int, default=30,
        help="How far back to search social media or trace (default: 30 days; for --trace, use 0 for all history)"
    )
    parser.add_argument(
    ...
    # --trace: build a full provenance tree
    if args.trace:
        if not args.claim and not args.url:
            print("Error: --trace requires --claim or --url")
            return
