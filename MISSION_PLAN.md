...
| 2026-09-16 | WHO-first presentation quick wins: the trace opener leads with the bolded source (date moves to the parenthesis); the WHO strip is promoted above the spread chart with click-to-highlight names (click a name, that source's dots light up); cards gain a cast deck under the age headline ("Origin: X · amplified by A, B +n") and the cast joins the OG description | Keep the date-led presentation; or jump straight to actor pages | Tarek's direction, echoing his 2026-07-05 "WHO > age" verdict: the mission is determining WHO creates and WHO amplifies, but the visuals had drifted WHEN-first — dates took the stage because role labels kept failing audits, and WHO-as-identity (names, presence, receipts — as robust as any date) was retreated from together with the fragile role layer by mistake. Age keeps the card headline (the Gate-2 share hook of record); the cast gets the deck. Actor pages (the cross-narrative actor surface) deliberately NOT slipped in: a real build whose entity-resolution errors would be misattribution — the most damaging error class — so it gets planned and sequenced against the Gate-2 clock as its own decision. |
| 2026-09-16 (later) | Trace cards go fully WHO-led: the ORIGIN NAME is the headline (eyebrow: 'Origin · <date>'), amplifiers + age + use-count ride the deck; age-led headline remains only as the fallback for traces with no nameable source. Supersedes the age-led card headline of 2a and the 'age keeps the headline' clause of this morning's row | Keep age-led with cast deck (this morning's compromise) | Tarek's explicit call on seeing the cards: 'more interested in the source and spreaders than the age.' Age stays visible (deck + chart label) but the artifact now answers WHO first — consistent with the mission statement and the WHO-first direction. Card set regenerated; og:title now names the source. |
| 2026-09-18 | Usage metrics, privacy-first: the Worker logs page views, search terms, and trace/contribution requests tagged with CDN-derived country/region/city — NEVER IP addresses (IPs only in 48h rate-limit counters); 180-day retention; maintainer-only /metrics endpoint + local metrics.py digest; disclosed in the site footer and METHODOLOGY ("the same standard of measurement we apply to narratives applies to ourselves") | Raw-IP logs; third-party analytics script; no measurement at all | Tarek needs usage/geo evidence (also Gate-2-adjacent: real usage IS other-people behavior), but a trust-focused provenance project storing visitor IPs would undercut its own posture. CF hands geo server-side, so the useful signal costs zero identity storage. Beacons are fire-and-forget and capped per IP so a stuck tab can't burn the KV free tier (~1k writes/day; Analytics Engine is the revisit path at scale). |
| 2026-09-18 (later) | Public trace requests now include the conceptual (idea) lineage: the request path runs `--evidence --conceptual` (~$0.70/request, DAILY_CAP 10 -> ceiling ~$7/day, was ~$0.40/~$4). The three already-served lean traces with idea-older-than-phrasing stories (Iroquois, AI-boys, Platner) upgraded in place via upgrade_conceptual.py — grafts ONLY the conceptual pass onto the published copies, preserving hand corrections | Keep the lean recipe and upgrade selectively on request; or full `--force` regeneration of served traces | Tarek's call after the spot-check discussion: phrasing-only traces understate exactly the traces where the idea is the story (seven-generations: the Haudenosaunee principle is centuries older than any modern phrasing). A phrasing-only answer to "where did this come from" is honest but incomplete. Cost stays bounded by the daily cap; in-place upgrade beats regeneration because the pipeline role-prompt fixes are still queued — a fresh lexical pass could reintroduce the exact errors the spot-check corrected. |
