...
**What can be contributed (v2, widened 2026-09-24 on early-reader feedback):**
- **A use we missed** — a URL, a date, and ideally the exact words used there, optionally labeled with its role in the spread (originator / early-amplifier / mass-amplifier / institutional-adoption / critic / mention; the entry records that the label was contributor-proposed). Any date is welcome (mid-history amplifiers matter, not just origins); if the date precedes the recorded earliest, the trace's "first attested" moves, credited to the contributor (and a `single-origin` status honestly downgrades to `earliest-found`).
- **A correction to an existing entry** — a structured proposal to change an entry's role, date, attribution, or receipt link, with a reason. Applied corrections change the field *and leave a dated note on the entry recording what changed, by whom, and why* — the same convention the maintainer's own corrections use. If a correction moves the earliest entry or removes an originator, the trace's origin fields and status follow.
- **Confirming or disputing an existing entry** — affirming an entry is right, or challenging it with a reason. Disputes mark the element `disputed` until resolved; they are sticky on purpose.
