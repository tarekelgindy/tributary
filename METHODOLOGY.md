...
- **A use we missed** — a URL, a date, and ideally the exact words used there. Any date is welcome (mid-history amplifiers matter, not just origins); if the date precedes the recorded earliest, the trace's "first attested" moves, credited to the contributor (and a `single-origin` status honestly downgrades to `earliest-found`).
